...
export default function CapApplicationForm({ onSubmitted }: Props) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto relative min-h-[500px] flex flex-col justify-center">
      ...
      <div
        className="relative z-10 rounded-3xl border border-zinc-200 dark:border-white/[0.08] overflow-hidden transition-all duration-500 bg-white/80 dark:bg-black/40 backdrop-blur-3xl shadow-2xl"
      >
        {/* Progress Bar */}
        {step > 1 && (
        ...
        <div className="p-8 sm:p-12 relative">
          ...
          <div className={`transition-all duration-500 ${step === 1 ? 'opacity-100 translate-x-0' : 'opacity-0 absolute inset-0 pointer-events-none translate-x-12'}`}>
            <div className="text-center space-y-6">
              ...
              <p className="text-lg text-zinc-500 dark:text-white/60 max-w-xl mx-auto leading-relaxed">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-8 pb-10">
                <div className="p-4 rounded-2xl bg-black/5 dark:bg-white/[0.03] border border-zinc-200 dark:border-white/[0.05] flex flex-col items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400"><Users size={20}/></div>
                  <div className="text-sm font-semibold text-zinc-900 dark:text-white">Host Seminars</div>
                </div>
                <div className="p-4 rounded-2xl bg-black/5 dark:bg-white/[0.03] border border-zinc-200 dark:border-white/[0.05] flex flex-col items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400"><Zap size={20}/></div>
                  <div className="text-sm font-semibold text-zinc-900 dark:text-white">Earn XP</div>
                </div>
                <div className="p-4 rounded-2xl bg-black/5 dark:bg-white/[0.03] border border-zinc-200 dark:border-white/[0.05] flex flex-col items-center gap-3">
                  ...
                  <div className="text-sm font-semibold text-zinc-900 dark:text-white">Unlock Perks</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative group">
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full px-5 py-4 rounded-2xl text-zinc-900 dark:text-white appearance-none outline-none transition-all duration-300 bg-black/5 dark:bg-white/5 border border-zinc-200 dark:border-white/10 hover:border-emerald-500/50 focus:border-emerald-500 focus:bg-white/10 focus:ring-4 focus:ring-emerald-500/10 cursor-pointer"
                  >
                    {GRAD_YEARS.map((y) => (
                      <option key={y} value={y} className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white">{y}</option>
                    ))}
                  </select>
                  <ChevronRight className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-600 dark:text-zinc-400 dark:text-white/30 group-hover:text-emerald-400 rotate-90 transition-colors pointer-events-none" />
                </div>
              </div>
            </div>
          ...
          <div className={`transition-all duration-500 ${step === 4 ? 'opacity-100 translate-x-0' : 'opacity-0 absolute inset-0 pointer-events-none -translate-x-12'}`}>
            ...
            <div className="mt-8 flex items-center justify-between">
              <button 
                onClick={handleBack}
                className="p-4 rounded-full bg-black/5 dark:bg-white/5 hover:bg-white/10 text-zinc-900 dark:text-white transition-colors"
              >
                <ChevronLeft />
              </button>
...
function StepHeader({ step, title, subtitle }: { step: string, title: string, subtitle: string }) {
  return (
    <div className="mb-10">
      ...
      <h2 className="text-3xl font-bold text-zinc-900 dark:text-white mb-2 tracking-tight">{title}</h2>
      <p className="text-zinc-500 dark:text-white/50">{subtitle}</p>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex justify-between items-end mb-2">
        <label className="block text-sm font-medium text-zinc-700 dark:text-white/70">{label}</label>
        {hint && <span className="text-xs text-zinc-600 dark:text-zinc-400 dark:text-white/30">{hint}</span>}
      </div>
      <div className="relative">
        <input
          ...
          className={`w-full px-5 py-4 rounded-2xl text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-white/20 outline-none transition-all duration-300 bg-black/5 dark:bg-white/5 border ${error ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/10' : 'border-zinc-200 dark:border-white/10 hover:border-emerald-500/50 focus:border-emerald-500 focus:bg-black/5 dark:focus:bg-white/10 focus:ring-4 focus:ring-emerald-500/10'}`}
        />
        {isFilled && !error && (
    ...
    <div>
      ...
      <div className="relative">
        <textarea
          ...
          rows={4}
          maxLength={maxLength}
          className={`w-full px-5 py-4 rounded-2xl text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-white/20 outline-none resize-none transition-all duration-300 bg-black/5 dark:bg-white/5 border ${error ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/10' : 'border-zinc-200 dark:border-white/10 hover:border-emerald-500/50 focus:border-emerald-500 focus:bg-black/5 dark:focus:bg-white/10 focus:ring-4 focus:ring-emerald-500/10'}`}
        />
        <div className="absolute bottom-4 right-4 flex items-center gap-2">
          {isFilled && !error && <CheckCircle2 className="text-emerald-500 w-4 h-4" />}
          <span className={`text-xs ${value.length >= maxLength ? 'text-red-400' : 'text-zinc-400 dark:text-white/20'}`}>
...
function NavigationButtons({ onBack, onNext }: { onBack: () => void, onNext: () => void }) {
  return (
    <div className="mt-8 flex items-center justify-between pt-6 border-t border-zinc-200 dark:border-white/5">
      <button 
        onClick={onBack}
        className="p-4 rounded-full bg-black/5 dark:bg-white/5 hover:bg-white/10 text-zinc-900 dark:text-white transition-colors"
      >
